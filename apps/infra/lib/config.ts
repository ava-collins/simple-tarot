...
export function loadInfraEnv(envFilePath = DEFAULT_ENV_FILE_PATH): NodeJS.ProcessEnv {
  if (!existsSync(envFilePath)) {
    throw new Error(`Missing infra environment file at ${envFilePath}. Create it from apps/infra/.env.example.`);
  }
...
function parseEnvironment(value: string): SimpleTarotEnvironment {
  ...
  throw new Error(`Unsupported Simple Tarot environment "${value}". Expected "dev" or "prod".`);
}
