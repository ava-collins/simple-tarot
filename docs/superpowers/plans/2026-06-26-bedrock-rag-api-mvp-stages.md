...
**Architecture:** Replace the current GraphQL/Neo4j `apps/graph-api` runtime with a new `apps/api` service. The API validates reading requests, builds a deterministic retrieval/generation prompt, calls Bedrock Knowledge Bases, and returns structured reading output with citations and evaluation metadata.
...
- Do not preserve the old `apps/graph-api` implementation as a runtime dependency.
...
**Purpose:** Make the new API understandable for future UI/hook integration while explicitly documenting what replaced `apps/graph-api`.
...
- Produces: cutover notes explaining why `apps/graph-api` is no longer the target backend.
